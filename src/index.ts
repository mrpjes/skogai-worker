...
const j = (obj: unknown, status = 200) =>
  new Response(JSON.stringify(obj), { status, headers: { ...JSON_HDR, "access-control-allow-origin":"*" } });

function base64FromBytes(u8: Uint8Array): string {
  let s = ""; const c = 0x8000;
  for (let i = 0; i < u8.length; i += c) s += String.fromCharCode.apply(null, Array.from(u8.subarray(i,i+c)) as any);
  // @ts-ignore
  return btoa(s);
}
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/" && req.method === "GET") {
      return new Response(JSON.stringify({ ok:true, msg:"SkogAI Worker: /upload, /process + public/index.html" }), { headers: JSON_HDR });
    }
...
${userPayload}`;
        ...
        if (!baseData) {
          ...
          if (inner) { try { baseData = JSON.parse(inner); } catch {} }
        }
        ...
        for (const name of analyses) {
          try {
            results[name] = (ANALYZERS as any)[name]
              ? (ANALYZERS as any)[name](ctx)
              : { ok:false, error:"unknown_analyzer" };
          } catch (e: any) {
    ...
    return new Response(JSON.stringify({ ok:true, msg:"UI + API running" }), { headers: JSON_HDR });
  }
};
